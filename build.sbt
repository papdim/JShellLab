...
description         := "A MATLAB-like environment for Java"

javaSource in Compile := { (baseDirectory in Compile)(_ / "src") }.value
...
createUberJar := {
  val bd = (baseDirectory in ThisBuild).value
  val log = streams.value.log
  val output = target.value / "JShellLabDL4j.jar"
  val classpathJars = Attributed.data((dependencyClasspath in Runtime).value)
