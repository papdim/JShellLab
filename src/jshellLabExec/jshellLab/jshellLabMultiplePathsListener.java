...
public class jshellLabMultiplePathsListener  implements TreeSelectionListener   {
    JTree  pathsTree;
  // the selectedValue keeps the full pathname of the selected object for further processing
    ...
    public jshellLabMultiplePathsListener(JTree classPathsTree) {
    ...
    public void explicitUpdate()  {
        ...
        // for nested objects, their path is repeated as the parent node, so concatenate the parent and the filename to build the complete path
          jshellLabMultiplePathsListener.selectedValues[k] = objPath[len-1].toString();
          jshellLabMultiplePathsListener.selectedPaths[k] = selectedValues[k].substring(0, selectedValues[k].lastIndexOf(File.separator));
      }
