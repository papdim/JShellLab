package jshellLabExec.ClassLoaders;

import jshellLabGlobal.Interpreter.GlobalValues;
import jshellLabExec.jshellLab.jshellLabToolbox;

import jshellLabExec.gui.AutoCompletion;
import java.io.*;
 ...
 return numGroovyLoadedClasses;
  ...
  public  int  scanAllBuiltInClasses(String jarFileName)    throws IOException 
  {
    ...
    String startupCode = null, patternsToAvoid = null;
        ...
        jshellLabExec.jshellLab.jshellLabToolbox  jshelllabToolbox = new jshellLabExec.jshellLab.jshellLabToolbox();
    ...
    if (toolboxInited == false)  {  // load the specified jar toolbox
        ...
        GlobalValues.jartoolboxesLoadedFlag.put(jarFileName, true);
         ...
         int toolboxIndex = 0;   // the jshellLabToolbox class keeps the name of the .jar file of each toolbox and a Vector that holds the bytecodes of its classes
           jshelllabToolbox.toolboxName = jarFileName;
           ...
           if (foundClass != null)  {
               ...
               if (Modifier.isAbstract(modifier)==false && Modifier.isInterface(modifier)== false &&  Modifier.isStatic(modifier)==false)    {  // class is not abstract or interface
                 if ( Modifier.isPublic(modifier) )  {  // class is public
                   ...
                   jshelllabToolbox.toolboxClasses.add(foundClass);
...
if (javaName.indexOf("$") == -1) {
 ...
 // construct a new AutoCompletion Object (needed in order to sort the list of methods, and to permit the access of the new methods)
            jshellLabGlobal.Interpreter.GlobalValues.AutoCompletionJShell= new jshellLabExec.gui.AutoCompletion();  // create the autocompletion object
 ...
 System.out.println("Total items of Autocompletion = "+ jshellLabGlobal.Interpreter.GlobalValues.AutoCompletionJShell.scanMethods.size());
