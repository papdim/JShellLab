...
public static boolean initAbbreviationsMap()   {
      ...
      if (abbreviationsMapInited == false) {
         abbreviationsMap =  buildAbbreviationsMap(jshellLabGlobal.Interpreter.GlobalValues.workingDir+java.io.File.separatorChar+"Abbreviations.txt");
...
// detects and returns the word at the current caret location
  public static  String  detectAndReplaceWordAtCaret()    {
     int caretPosition = jshellLabGlobal.Interpreter.GlobalValues.globalEditorPane.getCaretPosition()-1;
  
    String  txt = jshellLabGlobal.Interpreter.GlobalValues.globalEditorPane.getText();  // the whole editor's text
    ...
    while (offset >= 0 && exited==false) {
        ...
        jshellLabGlobal.Interpreter.GlobalValues.globalEditorPane.setSelectionStart(offset+1);
      ...
      for ( int i = length - 1 ; i >= 0 ; i-- )
        ...
        jshellLabGlobal.Interpreter.GlobalValues.globalEditorPane.setSelectionEnd(offset);
    ...
    if (abbreviationsMap.containsKey(wordAtCursor))  {
        String replacingText =  abbreviationsMap.get(wordAtCursor);
            ...
            jshellLabGlobal.Interpreter.GlobalValues.globalEditorPane.replaceSelection( replacingText);
    }
