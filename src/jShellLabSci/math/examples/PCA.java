package jShellLabSci.math.examples;
...
import static jShellLabSci.math.array.StatisticSample.*;
...
// de-normalization of y relatively to X mean and standard deviation
 ...
 private void view() {
  ...
  plot.addLinePlot(Math.rint(info[0] * 100 / jShellLabSci.math.array.StatisticSample.sum(info)) + " %", meanX, inv_center_reduce(U[0]));
  plot.addLinePlot(Math.rint(info[1] * 100 / jShellLabSci.math.array.StatisticSample.sum(info)) + " %", meanX, inv_center_reduce(U[1]));
 ...
 System.out.println("projection vectors\n" + DoubleArray.ToString(jShellLabSci.math.array.DoubleArray.transpose(U)));
 System.out.println("information per projection vector\n" + DoubleArray.ToString(info));
 }
 
 public static void main(String[] args) {
 double[][] xinit = jShellLabSci.math.array.StatisticSample.random(1000, 2, 0, 10);
