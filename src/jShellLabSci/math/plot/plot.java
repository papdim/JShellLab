...
import jShellLabSci.FFT.FFTResults;
import jShellLabSci.math.array.Matrix;
import jShellLabSci.math.array.Vec;
import jShellLabSci.math.plot.plotObjects.Axis;
import jShellLabSci.math.plot.plotObjects.LatexImage;
import jShellLabSci.math.plot.plotObjects.texLabel;
import jShellLabSci.math.plot.render.AbstractDrawer;
import java.util.Vector;

import static jShellLabSci.math.plot.utils.PArray.*;
...
// var x = vrand(500); plot(x);  markX(100, 200, new Font("Arial", Font.BOLD, 30));  markX(10.0, 0.3)
        static public void xlabel(String xLabelStr) {
            currentPlot.setAxisLabel(jShellLabSci.math.plot.PlotPanel.xAxisId, xLabelStr);
        }

        static public void ylabel(String yLabelStr) {
            currentPlot.setAxisLabel(jShellLabSci.math.plot.PlotPanel.yAxisId, yLabelStr);
        }

        static public void zlabel(String zLabelStr) {
            if (currentPlot instanceof  Plot3DPanel)
                currentPlot.setAxisLabel(jShellLabSci.math.plot.PlotPanel.zAxisId, zLabelStr);
        }
        ...
        public static void  latexLabel(String latex)  {
            texLabel  txl = new jShellLabSci.math.plot.plotObjects.texLabel(latex, 1, 1); 
            currentPlot.addPlotable(txl);
        }
        
        public static void  latexLabel(String latex, int coordx, int coordy)  {
            texLabel  txl = new jShellLabSci.math.plot.plotObjects.texLabel(latex, coordx, coordy); 
            currentPlot.addPlotable(txl);
        }
        
        public static void  latexLabel(String latex, int size,  int coordx, int coordy)  {
            texLabel  txl = new jShellLabSci.math.plot.plotObjects.texLabel(latex, size, coordx, coordy); 
            currentPlot.addPlotable(txl);
        }
        ...
        public static void latexLabel3d(String latex, double nw1, double nw2, double nw3, double se1, double se2, double se3, double sw1, double sw2, double sw3) {
            LatexImage tximg = new jShellLabSci.math.plot.plotObjects.LatexImage(latex, nw1,  nw2, nw3, se1, se2, se3, sw1, sw2, sw3);
            currentPlot.addPlotable(tximg);
        }
        
        public static void latexLabel3d(String latex, int xcoord, int ycoord) {
            LatexImage tximg = new jShellLabSci.math.plot.plotObjects.LatexImage(latex, xcoord, ycoord);
            currentPlot.addPlotable(tximg);
        }
...
static public PlotPanel plotv(Vector <Double>  x, Vector <Double> y, Vector <Double> z, Color color, String name) {
  ...
  // Apache Commons plotting
    jShellLabSci.math.plot.PlotPanel  plotApacheCommonsComplexArray( org.apache.commons.math3.complex.Complex  [] y) 
             {
        ...
        return jShellLabSci.math.plot.plot.plot(t, yreals,  color, plotType,  plotName, lineWidth); 
      }


    // plot FFT results, the real values
  static public jShellLabSci.math.plot.PlotPanel  plotfftReals( FFTResults  y) 
             {
              Color color = Color.GREEN;
              String plotName = "Real Values";
              int lineWidth = 1;
        ...
        return jShellLabSci.math.plot.plot.plot(t, y.realFFTs,  color, plotType,  plotName, lineWidth); 
      }

  // plot FFT results, the imaginary values
static public   jShellLabSci.math.plot.PlotPanel  plotfft( FFTResults  y)  {
     return plotfftReals(y);
  }
  
    // plot FFT results, the imaginary values
 static public  jShellLabSci.math.plot.PlotPanel  plotfftImags( FFTResults  y) 
             {
              Color color = Color.GREEN;
              String plotName = "Real Values";
              int lineWidth = 1;
        ...
        return jShellLabSci.math.plot.plot.plot(t, y.imFFTs,  color, plotType,  plotName, lineWidth); 
      }
