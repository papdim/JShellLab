...
import static jshellLabGlobal.Interpreter.GlobalValues.*;

import jShellLabSci.math.array.SvdResults;

import static net.NetSVD.*;

// jShellLabServer performs computations using socket based communication
public class  JShellLabNet  {
...
// init the JShellLab server
  public static void  initServer()   {  // init server with a seperate thread
    Thread initThread =   new Thread(() -> {
        try {
              GlobalValues.jshellLabServerSocket =  new ServerSocket( GlobalValues.jshellLabServerPort);
            System.out.println("Server Socket created ");
            Socket   saccepted = GlobalValues.jshellLabServerSocket.accept();   // accept a connection from the client
 ...
 // init a client that connects to a tremote jShellLab server
 public static void initClient() {
        try {
            //   System.out.println("before client socket creation");
            GlobalValues.sclient = new Socket(InetAddress.getByName(serverIP), GlobalValues.jshellLabServerPort);
            ...
            Logger.getLogger(JShellLabNet.class.getName()).log(Level.SEVERE, null, ex);
        }
 }
...
public static void initLocalHostClient() {
        try {
          //   System.out.println("before local host client socket creation");
            GlobalValues.sclient = new Socket(InetAddress.getLoopbackAddress(), GlobalValues.jshellLabServerPort);
           //  System.out.println("after client socket creation");
...
public static void specifyServerIP(String  IPOfjShellLabServer)  {
    serverIP  = IPOfjShellLabServer;
